...
char_ud <- char_ud %>% 
  mutate(characteristic_desc = recode(characteristic_desc, FSM_Eligible = "FSM_eligible"))
...
comparison_la_data_csv <- dplyr::select(
  ...
  mutate(area = ifelse(is.na(la_name) & is.na(region_name), "England",
                       ifelse(is.na(la_name), region_name,
                              la_name))) 
...
ukLocalAuthoritises <- shapefile("data/data_prep/England_LA_2016.shp")

exc_data <- filter(main_ud, level == 'Local authority', school_type == 'Total', year ==201617) %>%
